...
class PostcodeSuggestionsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            response = requests.get(f'https://api.postcodes.io/postcodes/{query}/autocomplete')
            data = response.json()

            if response.status_code == 200 and data.get('result'):
                suggestions = data['result']
                logger.info(f'Found {len(suggestions)} postcode suggestions for query: {query}')
    ...
    def get(self, request):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f'Found {len(predictions)} address predictions for: {input_text}')
...
def google_place_details(request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            parsed_address = parse_google_address_components(result.get('address_components', []))
    ...
    try:
        ...
        for addr in google_addresses:
            all_addresses.append({
                ...
                'main_text': addr.get('structured_formatting', {}).get('main_text', ''),
                'secondary_text': addr.get('structured_formatting', {}).get('secondary_text', ''),
            })
...
def postcode_address_lookup_enhanced(request):
    ...
    try:
        ...
        logger.info(f'Found {len(unique_addresses)} unique addresses for postcode: {postcode}')
        ...
        return JsonResponse({
            ...
            'recommendations': {
                'use_ideal_postcodes': len(ideal_addresses) == 0 and len(unique_addresses) < 5,
                'manual_entry_required': len(unique_addresses) == 0
            }
        })

    except Exception as e:
        logger.error(f'Error in enhanced postcode address lookup: {str(e)}', exc_info=True)
        return JsonResponse({'error': 'Address lookup failed'}, status=500)
            ...
            if geocode_data.get('status') == 'OK' and geocode_data.get('results'):
                ...
                if nearby_response.status_code == 200:
                    ...
                    if nearby_data.get('status') == 'OK':
                        return nearby_data.get('results', [])[:15]  # Limit to 15 results
...
def simple_postcode_addresses(request):
    ...
    try:
        ...
        logger.error(f'Error in simple postcode address lookup: {str(e)}', exc_info=True)
